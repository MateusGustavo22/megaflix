...
export default function MovieDetails(props: Props) {
  ...
  const [sinopse, setSinopse] = useState(
    props.sinopse.length >= 200
      ? props.sinopse.substring(0, 200) + " . . ."
      : props.sinopse
  );
    ...
    <div className="max-w-[600px] flex-row pl-8 pr-4 mb:pl-4 z-5">
      <div className="w-full flex-row space-y-8 ">
        <div className="w-full h-full flex-row space-y-5">
          <h1 className="text-4xl mb:text-3xl font-bold text-white">
            {props.name}
          </h1>
          <div className="w-full min-h-[50px overflow-hidden">
            <p className="text-white text-base font-regular leading-loose max-w-full">
              {sinopse}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 mb-4 block">
          <Link href={`/player?id=${props.id}`}>
            <button className="max-w-xs w-full h-16 mb:max-w-full rounded-md font-bold text-lg hover:shadow-blueShadow hover:shadow-3xl text-white bg-button">
              Assistir Agora
            </button>
          </Link>
        </div>
