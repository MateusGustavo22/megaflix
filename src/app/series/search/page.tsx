...
export default async function SerieSearch() {
  const apiKey = process.env.NEXT_PUBLIC_API_KEY;
  const params = useSearchParams();
  const movieQuery = params.get("q");
  const searchUrl = `${process.env.NEXT_PUBLIC_SERIE_SEARCH}api_key=${apiKey}&query=${movieQuery}`;
  ...
  return (
    <div className="w-full h-full flex-row space-y-4 mt-8">
      <span className="text-gray-300 block m-auto text-center font-medium text-lg">
        Resultados para:{" "}
        <span className="text-white font-semibold">{movieQuery}</span>
      </span>
